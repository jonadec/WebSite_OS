...
vector<int> bubbleSort(vector<int>& vec) {
    ...
    return vec; // Devolver el vector ordenado
}
...
int main() {
    ...
    while (true) {
        ...
        if (choice == 1) {
           ...
           //vector<int> partitionsOrdered(numPartitions) =
            vector<int>partitionordered=bubbleSort(partitionSizes);
            for (int i = 1; i < numPartitions; i++) {

               
                if (memory[i].name.empty() && partitionordered[i] >= process.size) {
                    memory[i] = process;
                    allocated = true;
